...
async function parseSession(file, stat) {
  ...
  // A rollout is append-only and gets resumed, so the session producing today's
  // tokens may have started days ago. Dating the row by the session start filed
  // that work under the wrong day and made this card look frozen, so the last
  // usage record's own timestamp wins.
  const usageAt = usageRecord?.timestamp ?? tokenCount?.timestamp ?? startedAt;
  return {
    date: dayKey(new Date(usageAt)),
    startedAt,
    usageAt,
    cwd: meta?.cwd ?? null,
...
export async function collect({ days = 60 } = {}) {
  ...
  for (const file of files) {
    ...
    if (value.rateLimits?.metrics?.length) {
      // The record carries its own timestamp. Ranking by session start threw
      // away the newest limits whenever they arrived in a resumed session.
      const at = Date.parse(value.rateLimits.at ?? value.usageAt ?? value.startedAt) || 0;
      if (at >= latestRateAt) {
